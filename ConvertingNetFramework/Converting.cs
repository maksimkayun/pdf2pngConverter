            ...
            if (pdfFile != null)
            {
                ...
                using (var rasterizer = new GhostscriptRasterizer()) //create an instance for GhostscriptRasterizer
                {
                    ...
                    for (int i = 0; i < numberOfPages; i++)
                    {
                        ...
                        using (var pdf2PNG = rasterizer.GetPage(dpi, i + 1))
                        {
                            using (Bitmap bitmap = new Bitmap(pdf2PNG))
                            {
        ...
        public static void Pngs2Pdf(List<FileInfo> imagePaths)
        {
            ...
            if (directoryFullName != null)
            {
                PdfWriter pdfWriter = PdfWriter.GetInstance(pdfDoc,
                    new FileStream(Path.Combine(directoryFullName, "output.pdf"), FileMode.Create));
            }
            ...
            {
                var img = Image.GetInstance(image.FullName);
                img.ScaleAbsolute(pdfDoc.PageSize);
